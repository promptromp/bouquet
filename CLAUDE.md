...
- **Task queue** — Pluggable backend system (`TaskQueueBackend` ABC in `src/bouquet/tasks/base.py`). `LocalBackend` uses SQLite (`~/.local/state/bouquet/{project}.tasks.db`, `check_same_thread=False`). Backend is synchronous; TUI wraps calls in `@work(thread=True)`. Task ID is always a string (unifies SQLite rowids and future GitHub issue numbers). Factory in `src/bouquet/tasks/__init__.py` creates the backend from `TaskQueueConfig`. GitHub backend is Phase 2.
...
- **Task TUI keybindings** — `c` creates a task (opens `CreateTaskScreen` modal), `x` picks up the highlighted task in `TaskQueueTable`. Task table refreshes on mount, after create, after pickup, and on `r`.
