...
- **Bootstrap setup commands** — `BootstrapConfig.setup_commands` runs shell commands in a single bash context before dependency installation. Each command string is rendered through `template.render_template()` (`{{ BOUQUET_WORKTREE_INDEX }}` etc.) and the same vars are also exported into the bash subprocess as plain shell env vars (`$BOUQUET_*`), so callers can pick whichever form reads better. Env vars exported by the user's commands are captured (via a `python3` JSON dump to a temp file — portable across macOS/Linux) and propagated to deps install subprocesses and the tmux session (via `set_environment`). The captured-delta diff is scoped against `subproc_env` (not `os.environ`) so the injected `BOUQUET_*` vars cancel out and never leak into the delta. **Failures are loud:** if any command exits non-zero, the captured stdout/stderr are written to bouquet's stderr **and to the file logger** (next bullet) and `SetupCommandsError` (a `RuntimeError` subclass) is raised — this propagates up through `WorktreeManager._setup_window` so the worktree is marked `ERROR` rather than silently left in a partially-bootstrapped state. The bash script is structured as `<user_cmds>; _BOUQUET_USER_RC=$?; <env_dump>; exit $_BOUQUET_USER_RC` so the env-dump still runs (best-effort capture of partial exports) but the user's exit code is what propagates. Use cases: private registry auth (e.g. AWS CodeArtifact tokens), per-worktree resource provisioning (databases, message queues, cache namespaces).
- **post_deps_commands** — `BootstrapConfig.post_deps_commands` runs after `python_deps_command`/`node_deps_command`, before `direnv_allow`. Same engine as `setup_commands` (`_run_setup_and_capture_env` with `phase="post_deps_commands"`), same env-delta semantics — the captured delta is merged into the dict returned by `bootstrap_worktree` and propagated to the tmux session like setup's. **Same loud-failure semantics**: a non-zero exit raises `SetupCommandsError` (with `phase` in the message) so `WorktreeManager.create()` marks the worktree `ERROR`. Use case: anything that needs the venv/`node_modules` to exist before it runs — typical example is `uv run migrate upgrade head` per fresh worktree.
- **teardown_commands** — `BootstrapConfig.teardown_commands` runs in `WorktreeManager.remove()` BEFORE killing tmux and removing the git worktree (so user code can still reach the on-disk checkout). Same engine, `phase="teardown_commands"`. **Best-effort, NOT loud:** failure logs a warning via the bouquet logger and continues with tmux/git cleanup — rationale being that a transient infra failure (external service down) shouldn't block local cleanup, which the user can always retry. Use case: reclaiming external per-worktree resources at remove time (databases, queues, cache namespaces).
- **File logging** (`bouquet/log.py`) — `bouquet_log.configure(project_name, level)` is called early in `cli.py` (`bouquet start`/`stop`) and `tui/app.py:_run_tui`. Attaches a `RotatingFileHandler` (5 MB × 3 backups) at `~/.local/state/bouquet/{project}.log` to the `bouquet` parent logger, plus a stderr handler at WARNING+ for direct CLI use. Idempotent (skip if a file handler is already attached for the same path). `bouquet_log.LOG_FILE` exposes the active path so `SetupCommandsError`'s message can include it (`"setup_commands failed … — see /…/proj.log for details"`) — this is what makes the TUI's terse error toast actionable, since stdout/stderr are captured by Textual but the file log is durable. All bouquet modules use `logging.getLogger(__name__)` and emit through the configured `bouquet` parent (`propagate=False` so no double-log via root). `--log-level` flag on `bouquet start` controls file-handler verbosity; default INFO.
